...
const sendRequest = async (unitId: any = unitUserTwoId, token: string = '') => {
  const agent = request(app).get(`/api/v1/units/${unitId}`);
...
describe('Get single unit', () => {
  ...
  it('Should return 401 unauthorized when wrong auth token is provided', async () => {
    const response = await sendRequest('eadsa31s');
  ...
  it('Should return unit id and unit name when valid request is sent', async () => {
    const response = await sendRequest(unitUserTwoId, userTwoToken);

    expect(response.body.unit.id).toBe(unitUserTwoId.toString());
    expect(response.body.unit.name).toBe(unitUserTwoName);
  });
});
